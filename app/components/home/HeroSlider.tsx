...
export default function HeroSlider({ drupalSlides }: HeroSliderProps) {
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden bg-[var(--atut-navy)]">
      <Swiper
          ...
          return (
            ...
            >
              ...
              <div className="absolute inset-0 z-30 flex justify-center items-center pointer-events-none">
                ...
                >
                  <span className="text-[var(--atut-red)] font-mono text-xs md:text-base uppercase tracking-widest mb-4 block drop-shadow-[0_2px_4px_rgba(0,0,0,0.9)]">
                    {slide.subtitle}
                  </span>
                  ...
                  <div className="w-12 md:w-16 h-[3px] md:h-[4px] bg-[var(--atut-red)] mt-6 mb-6 md:mt-8 md:mb-8 drop-shadow-[0_2px_4px_rgba(0,0,0,0.5)]"></div>
                  {slide.sektorUrl && (
                    <button type="button" onClick={() => router.push(slide.sektorUrl!)} className={`inline-block bg-[var(--atut-red)] text-white font-bold text-[10px] md:text-xs uppercase tracking-widest px-5 py-2 md:px-8 md:py-4 hover:bg-white hover:text-gray-950 transition-all duration-300 cursor-pointer relative z-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-white ${isCurrent ? "pointer-events-auto" : "pointer-events-none"}`}>
                      Nasza Oferta
                    </button>
                  )}
                </div>
      ...
      >
        <div className="w-5 h-10 border-2 border-[var(--atut-red)] rounded-full flex justify-center pt-1.5">
          <div className="w-1 h-3 bg-[var(--atut-red)] rounded-full animate-telemetry"></div>
        </div>
        <span className="text-[10px] font-mono font-bold text-[var(--atut-red)] uppercase tracking-widest drop-shadow-md">
          Przewiń
        </span>
      </div>

      {/* Nawigacja kropki — obszar dotyku 24px (WCAG 2.5.8), kreska wizualnie cienka */}
        ...
        {slides.map((_, index) => {
          ...
          return (
            <button
              ...
              className="group relative h-6 w-8 md:w-10 flex items-center z-40 cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--atut-red)]"
            >
              <span className="relative block h-1.5 md:h-2 w-full overflow-hidden bg-slate-700/60 transition-all group-hover:bg-gray-400/60">
                <span
                  ref={isCurrent ? progressLine : null}
                  className={`absolute left-0 top-0 h-full bg-[var(--atut-red)] ${isCurrent ? "opacity-100" : "opacity-0 w-0"}`}
                />
              </span>
            </button>
          );
