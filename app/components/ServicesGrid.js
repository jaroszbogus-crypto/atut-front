...
const iconLibrary = {
  ...
  scada: (
    <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
      <path strokeLinecap="round" strokeLinejoin="round" d="M9 17.25v1.007a3 3 0 01-.879 2.122L7.5 21h9l-.621-.621A3 3 0 0115 18.257V17.25m6-12V15a2.25 2.25 0 01-2.25 2.25H5.25A2.25 2.25 0 013 15V5.25A2.25 2.25 0 015.25 3h13.5A2.25 2.25 0 0121 5.25zM9 7.5l3 3m0 0l3-3m-3 3v6" />
    </svg>
  ),
  bezpieczeństwo: (
    <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
  ...
  modernizacja: (
    <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
      <path strokeLinecap="round" strokeLinejoin="round" d="M8.25 3v1.5M4.5 8.25H3m1.5 7.5H3m15-7.5h1.5m-1.5 7.5H1.5M8.25 19.5V21m7.5-18v1.5m0 15V21m-12-3h12a2.25 2.25 0 002.25-2.25V5.25A2.25 2.25 0 0017.25 3H6.75A2.25 2.25 0 004.5 5.25v10.5A2.25 2.25 0 006.75 18z" />
    </svg>
  ),
  serwis: (
    <svg fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
      <path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12a7.5 7.5 0 0015 0m-15 0a7.5 7.5 0 1115 0m-15 0H3m16.5 0H21m-1.5 0H12m-8.457 3.077l1.41-.513m14.095-5.128l1.41-.513M5.106 17.785l1.15-.827m11.379-8.16l1.15-.827M8.14 21.27l.707-1.03m7.45-.808l.707-1.03M12 3v1.5m0 15V21m4.743-10l-1.149-.827" />
    </svg>
  )
};
...
export default function ServicesGrid({ drupalHeader, drupalServices }) {
  ...
  const sectionHeader = {
    ...
    title: drupalHeader?.title || "Kompleksowe usługi dla automatyki przemysłowej"
  };

  // 2. Domyślna lista usług (fallback), jeśli tablica z Drupala jest pusta
  const defaultServices = [
    { num: "01", iconKey: "aparat", title: "Aparatura modułowa i pomiary", desc: "Dobór i montaż zabezpieczeń, aparatury rozdzielczej oraz wykonywanie pełnych powdrożeniowych pomiarów skuteczności ochrony przeciwporażeniowej." },
    { num: "02", iconKey: "naped", title: "Układy napędowe i sensoryka", desc: "Integracja falowników, serwonapędów oraz zaawansowanych czujników optycznych, laserowych i indukcyjnych do precyzyjnej kontroli pozycji i prędkości." },
    { num: "03", iconKey: "scada", title: "Systemy SCADA / HMI", desc: "Projektowanie intuicyjnych interfejsów graficznych dla operatorów maszyn oraz nadrzędnych systemów monitorowania i akwizycji danych produkcyjnych." },
    { num: "04", iconKey: "bezpieczeństwo", title: "Bezpieczeństwo maszyn (LOTO)", desc: "Audyty bezpieczeństwa parków maszynowych, dostosowanie układów sterowania do dyrektywy maszynowej, montaż rygli bezpieczeństwa i barier optycznych." },
    { num: "05", iconKey: "modernizacja", title: "Retrofitting i Modernizacja", desc: "Kompleksowa wymiana przestarzałych sterowników PLC i układów przekaźnikowych na nowoczesne systemy sterowania bez konieczności wymiany całej maszyny." },
    { num: "06", iconKey: "serwis", title: "Wsparcie i Utrzymanie Ruchu", desc: "Szybka diagnostyka błędów w programach PLC, testy pętli sygnałowych na obiekcie oraz stała asysta techniczna dla ciągłości Twojej produkcji." }
  ];

  // Jeśli Drupal dostarczy tablicę, używamy jej, w innym wypadku bierzemy dane domyślne
  const servicesList = drupalServices && drupalServices.length > 0 ? drupalServices : defaultServices;
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 border-t border-l border-gray-200">
          {servicesList.map((service, index) => {
            ...
            const selectedIcon = iconLibrary[service.iconKey] || iconLibrary["serwis"];
            
            // Formatowanie numeru indeksu (jeśli Drupal nie przyśle sztywnego "num", wyliczamy go automatycznie z kolejności)
            const displayNum = service.num || String(index + 1).padStart(2, '0');
